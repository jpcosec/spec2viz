...
from yaml_charts.ir import SequenceIR, StateIR, ComponentIR, ActivityIR, DeploymentIR
...
class MermaidRenderer:
    ...
    def _sequence(self, ir: SequenceIR) -> str:
        ...
        for msg in ir.messages:
            ...
            lines.append(f"    {msg.from_}{arrow}{msg.to}: {msg.message}")
        return "\n".join(lines)

    def _state(self, ir: StateIR) -> str:
        lines = ["stateDiagram-v2", f"    [*] --> {ir.initial}"]
        ...
        for step in ir.steps:
            if step.kind == "decision":
                for branch, target in step.branches.items():
                    ...
                    lines.append(f"    {step.id} -->|{branch}| {dest}")
            elif step.next:
    ...
    def _deployment(self, ir: DeploymentIR) -> str:
        ...
        for node in ir.nodes:
            ...
            for aid in node.contains:
                if aid in art_map:
                    lines.append(f"        {aid}[\"{art_map[aid].label}\"]")
            lines.append("    end")
        for conn in ir.connections:
            label = conn.protocol or ""
            if label:
                lines.append(f"    {conn.from_} -->|{label}| {conn.to}")
            else:
